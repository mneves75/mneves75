...
**STORY:** Visitors read a statement backed by a live-looking session of verifiable facts, inspect constraints behind selected work, browse the work index (current work first, stage badges on anything not yet live, an archive below), then choose public proof or a channel.
...
- Terminal panels are real HTML text — selectable, screen-reader-sane, never images or ASCII-art screenshots. Panels are flat (1px border, no drop shadow) and the bar carries one copper status square, not faux window dots.
- v3.1 (2026-09): project imagery is full colour at 16:10 (1600×1000 WebP, real product screens or the live page — no grayscale filter, no abstract art). Release stage is a bordered mono text badge (`.stage-badge`), never colour alone. Removed as dated: cursor spotlight on the hero, nav hover scramble. The hero terminal lists work in motion (`ls work/ --in-motion`), so the selected-work ledger is the only featured list.
- Motion: opt-in behind `html[data-motion='on']` (set only when reduced motion is not preferred). Entries 280–700ms with `--ease-out`, hovers 180ms, lifts ≤2px. Scroll reveals via `.reveal` + IntersectionObserver; masthead stretch via native `animation-timeline: view()` only. Cross-document view transitions (`@view-transition`) guarded by `prefers-reduced-motion`. Reduced-motion users get everything visible and still. Whenever the marquee is not animating (paused, reduced motion or no JS) it becomes a scrollable strip. Every looping or auto-updating device (decoder, marquee, signal trace, cursor blink) also stops through the header pause control (`data-motion-toggle`, persisted as `mn-motion`), required by WCAG 2.2.2; JS devices check `data-motion` on each tick.
