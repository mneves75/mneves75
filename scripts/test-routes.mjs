...
assert.equal((work.match(/data-project-row data-category/g) ?? []).length, 27, 'portfolio project count changed; verify source inventory before editing');
assert.equal((ptWork.match(/data-project-row data-category/g) ?? []).length, 27, 'Portuguese portfolio project count changed; verify source inventory before editing');
assert.match(work, /Bolão 2026/);
assert.match(bolao, /Free web app for 2026 World Cup predictions/);
assert.equal((sitemap.match(/<loc>/g) ?? []).length, 64, 'sitemap URL count changed; verify all locale routes');
