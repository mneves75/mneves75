...
assert.equal((work.match(/data-project-row data-category/g) ?? []).length, 33, 'portfolio project count changed; verify source inventory before editing');
assert.equal((ptWork.match(/data-project-row data-category/g) ?? []).length, 33, 'Portuguese portfolio project count changed; verify source inventory before editing');
...
assert.equal((sitemap.match(/<loc>/g) ?? []).length, 76, 'sitemap URL count changed; verify all locale routes');
