...
assert.equal((work.match(/data-project-row data-category/g) ?? []).length, 31, 'portfolio project count changed; verify source inventory before editing');
assert.equal((ptWork.match(/data-project-row data-category/g) ?? []).length, 31, 'Portuguese portfolio project count changed; verify source inventory before editing');
...
assert.equal((sitemap.match(/<loc>/g) ?? []).length, 72, 'sitemap URL count changed; verify all locale routes');
